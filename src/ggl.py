...
class GGL(object):
    ...
    def run_treecorr_jackknife(self, lens, source, type_corr):
        ...
        def run_jki(jk):
            ...
            if type_corr == 'NG':
                ...
                corr = treecorr.NGCorrelation(nbins=self.config['nthbins'], min_sep=self.config['thlims'][0], max_sep=self.config['thlims'][1], sep_units='arcmin', bin_slop=self.config['bslop'])
    
            if type_corr == 'NN': 
                if jk==0: print 'Doing NN correlation.'
                corr = treecorr.NNCorrelation(nbins=self.config['nthbins'], min_sep=self.config['thlims'][0], max_sep=self.config['thlims'][1], sep_units='arcmin', bin_slop=self.config['bslop'])
    

            if len(ra_l_jk) > 1:

                if len(ra_l_jk)%2==0: # If len array is even, make it odd before computing the median.
                    pixjk = hp.ang2pix(nside,((90.0 - np.median(np.append(dec_l_jk,0)))*np.pi/180.),np.median(np.append(ra_l_jk,0))*np.pi/180.0)
                if len(ra_l_jk)%2==1: # If len array is odd, keep it odd.
                    pixjk = hp.ang2pix(nside,((90.0 - np.median(dec_l_jk))*np.pi/180.),np.median(ra_l_jk)*np.pi/180.)
                ...
                cat_s = treecorr.Catalog(ra=ra_s[bool_s], dec=dec_s[bool_s], g1=e1[bool_s], g2=e2[bool_s], w=w[bool_s], ra_units='deg', dec_units='deg')
    ...
    def run_nk(self, lens, source):
        ...
        nk = treecorr.NKCorrelation(nbins=self.config['nthbins'], min_sep=self.config['thlims'][0], max_sep=self.config['thlims'][1], sep_units='arcmin', bin_slop=self.config['bslop'])
	...
	"""
        ...
        for i in range(len(gth_all)):
            Ah, chi2fit_h[i], _ = functions.minimize_chi2_fit_amplitude((cov_sims[mask].T)[mask].T, gth_all[i][mask], sims[mask])
            Al, chi2fit_l[i], _ = functions.minimize_chi2_fit_amplitude((cov_sims[mask].T)[mask].T, gtl_all[i][mask], sims[mask])
            ratioA_all[i] = Ah/Al
...
class Measurement(GGL):
    ...
    def plot_boostfactors(self):
        ...
        for l in range(0, len(zbins['lbins'])):
            for s in range(len(zbins['sbins'])):
                ...
                if l == 0:
                    ax[s][l].set_ylabel('%s\n'%plotting['redshift_s'][s] + r'Boost factors',size='larger', linespacing = 3)
                if s == 0:
    ...
    def plot_randoms(self):
        ...
        for l in range(0, len(zbins['lbins'])):
            ...
            for s in range(len(zbins['sbins'])):
                ...
                if l == 0:
                    ax[s][l].set_ylabel('%s\n'%self.plotting['redshift_s'][s] + r'$\gamma_t (\theta)$',fontsize=fs, linespacing = 3)
    ...
    def plot_gammax(self):
        ...
        print len(chi2s)
        ax[1].hist(chi2s, bins = 6, color = colors[c], ec = colors[c], lw = 2, normed = True, histtype = 'step', alpha = 1, label = labels[c])

        # Chi2 distribution
        ndf = len(gx)
        t = np.linspace(0., 60, 300)
        ax[1].fill_between(t,0,scipy.stats.chi2.pdf(t, ndf) , color='gray', alpha = 0.5, label='$\chi^2$ pdf (ndf$ = %d$)'%ndf)
        ax[1].set_ylim(0,0.1)
    ...
    def plot(self, lens_random):
        ...
        for l in range(0, len(zbins['lbins'])):
            ...
            for s in range(len(zbins['sbins'])):
                ...
                ax[s][l].plot(theta, [np.mean(R_nk)]*len(theta), ':', lw=2, color = c1, mec = c1, label = r'$\overline{R_{\mathrm{nk}}}$')
                ax[s][l].set_xscale('log')
                ax[s][l].set_xlim(self.config['thlims'][0], self.config['thlims'][1])
                ...
                ax[s][l].text(0.5,0.85,r'$\Delta R/R = %0.2f \%%$'%(100*np.mean((R_nk - R_mean)/(R_mean+R_nk)*2)),
                              horizontalalignment='center', verticalalignment='center', transform=ax[s][l].transAxes, fontsize = 'larger')
...
class TestStars(GGL):
    ...
    def plot(self):
        ...
        for t, type in enumerate(types):
            ...
            if s == 0:
                ax.errorbar(th*(1+0.05*(shifts[t])), gt, err, fmt = styles[t], color = colors[s], mec = colors[s], label = titles_c[s] + ' ' + type, markersize=4.5)
            if s==1:
                ax.errorbar(th*(1+0.05*(shifts[t+2])), gt, err, fmt = styles[t], color = colors[s], mec = colors[s], label = titles_c[s] + ' ' + type, markersize=4.5)
...
class TestSizeSNR(GGL):
	...
	"""
        ...
        nzsl = interpolate.interp1d(zs+self.source_nofz_pars['dzs',size_snr][0],nzsl,bounds_error=False,fill_value=0)(zs)
        nzsh = interpolate.interp1d(zs+self.source_nofz_pars['dzs',size_snr][1],nzsh,bounds_error=False,fill_value=0)(zs)
	...
	self.save_runs(path_test, theta, gtsl, gxsl, errsl, weightsl, npairsl, False)
        ...
        theta, gtsl_r, gxsl_r, errsl_r, weightsl_r, npairsl_r = self.run_treecorr_jackknife(random, sources[maskl], 'NG')
	self.save_runs(path_test, theta, gtsl_r, gxsl_r, errsl_r, weightsl_r, npairsl_r, True)
        ...
        theta, gtsh_r, gxsh_r, errsh_r, weightsh_r, npairsh_r = self.run_treecorr_jackknife(random, sources[maskh], 'NG')
	...
	fig.subplots_adjust(hspace=0.32,wspace=0.0)
	    ...
	    ax[0][t].set_xlabel('Redshift')
            ax[0][t].plot(zs+self.source_nofz_pars['dzs',size_snr][0],nzsl,ls='-', lw=1.5, c=color, label = 'Low %s'%(labels_c[0]))
            ax[0][t].plot(zs+self.source_nofz_pars['dzs',size_snr][1],nzsh,ls='-.', lw=1.5, c=color, label = 'High %s'%(labels_c[0]))
        ...
        for t, size_snr in enumerate(tests):
            ...
            ax[1][t].fill_between(x_lin, np.array([invsc - err_invsc for i in x_lin]), 
                              np.array([invsc + err_invsc for i in x_lin]), alpha=0.4, edgecolor = color, facecolor = color, 
                              label='$\Sigma_{\mathrm{crit,eff}}^{-1,\,\mathrm{high}}/ \Sigma_{\mathrm{crit,eff}}^{-1,\,\mathrm{low}}$ %s'%(labels_c[0]))
...
class TestSysMaps(GGL):
    ...
    def load_systematics_map(self, map, band, nside, nested_bool):
        ...
        sys_map = pf.getdata(path + 'Y1A1NEW_COADD_SPT_band_%s/'%band+ 'Y1A1NEW_COADD_SPT_band_%s_nside%s_oversamp4_'%(band, nside) + self.sysmaps[map] + '.fits')
        pix = sys_map['PIXEL']
        sig = sys_map['SIGNAL']
	...
	"""
        zl, nzl = np.loadtxt(self.paths['nz_lens'] + 'lens', unpack=True,usecols=(0,1))
        zs, nzsl, nzsh = np.loadtxt(self.paths['y1base'] + 'runs/test_mcal_bpzmof_unblind/nofzs/source_%s_%s_notomo'%(map, band),unpack=True)
        return zl, nzl, zs, nzsl, nzsh
    ...
    def run(self, maps, bands):
        ...
        for map in maps:
            ...
            for band in bands: 
                ...
                pix_low, pix_hi = self.load_systematics_map(map, band, self.sysmaps['nside'], self.sysmaps['nested_bool'])
                ...
                # Computing the measurements for the split halves, both around lenses and randoms
                print 'Lenses, low.'
                theta, gtsl, gxsl, errsl, weightsl, npairsl = self.run_treecorr_jackknife(lens[maskl_low], sources[masks_low], 'NG')
                gtlnum, gxlnum, wlnum = self.numerators_jackknife(gtsl,gxsl,weightsl)

                print 'Randoms, low.'
                theta, gtsl_r, gxsl_r, errsl_r, weightsl_r, npairsl_r = self.run_treecorr_jackknife(random[maskr_low], sources[masks_low], 'NG')
                gtlnum_r, gxlnum_r, wlnum_r = self.numerators_jackknife(gtsl_r,gxsl_r,weightsl_r)

                print 'Lenses, high.'
                theta, gtsh, gxsh, errsh, weightsh, npairsh = self.run_treecorr_jackknife(lens[maskl_hi], sources[masks_hi], 'NG')
                gthnum, gxhnum, whnum = self.numerators_jackknife(gtsh,gxsh,weightsh)

                print 'Randoms, high.'
                theta, gtsh_r, gxsh_r, errsh_r, weightsh_r, npairsh_r = self.run_treecorr_jackknife(random[maskr_hi], sources[masks_hi], 'NG')
                gthnum_r, gxhnum_r, whnum_r = self.numerators_jackknife(gtsh_r,gxsh_r,weightsh_r)
    ...
    def plot(self):
        ...
        for m, map in enumerate(maps):
            ...
            ax[f][c].errorbar(x, data, data_err, fmt = 'o', color = color,mec = color, label = '%s'%(self.plotting['metacal']))
            x_lin = [x-0.2,x,x+0.2] 
            ax[f][c].fill_between(x_lin, np.array([theory - theory_err for i in x_lin]), 
                                  np.array([theory + theory_err for i in x_lin]), alpha=0.4, edgecolor = color, facecolor = color, 
                                  label=r'$\Sigma_{\mathrm{crit,eff}}^{-1,\,\mathrm{high}}/ \Sigma_{\mathrm{crit,eff}}^{-1,\,\mathrm{low}} %s$'%(labels_cshort))
            ...
            ax[f][c].text(0.5,0.9, r'\textsc{%s}'%map,
                          horizontalalignment='center', verticalalignment='center', transform=ax[f][c].transAxes, fontsize = 12, color = 'k')
...
if run_psf:
    ...
    ra_lims = (-1,361)
    dec_lims = (-90,-35)
    psf.save_psf_residuals(ra_lims, dec_lims)
    psf.run()
    psf.plot()
