...
def jk_centers(ra_r, dec_r, path):
    ...
    km = kmeans_radec.kmeans_sample(
        radec_r, config['njk'], maxiter=500, tol=1e-05)
    # Did we converge?
...
def plot_jk(paths, config, ra, dec, jk):
    ...
    proj = createConicMap(
        ax, ra, dec, proj_class=AlbersEqualAreaProjection, bgcolor='w')
    x, y = proj(ra, dec)
    marker = 's'
    markersize = getMarkerSizeToFill(fig, ax, x, y)
    sc = ax.scatter(x, y, c=jk, edgecolors='None', marker=marker,
                    s=markersize, cmap=cmap, rasterized=True, zorder=1)
